...
const InventorySalesInvoiceTable = ({
  ...
  const columns = [
    // Number column with special styling
    {
      field: "number",
      header: "INVOICE #",
      width: "150px",
    ...
    // Customer column
    {
      ...
      width: "200px",
      render: (value) => (
        <span className="text-[#1f221f] font-medium">{value || "Walk-in Customer"}</span>
      )
    },
    // Status column
    {
  ...
  // Add any additional columns from schema that aren't already included
  if (schema?.table_columns) {
    schema.table_columns.forEach((key) => {
      // Skip columns we already have
      if (!["number", "date", "customer_name", "status", "subtotal", "vat", "total"].includes(key)) {
        columns.push({
          field: key,
        ...
        <Table
          title={
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-semibold text-[#1f221f]">{schema?.name || "Inventory Sales Invoices"}</h2>
                <p className="text-xs text-[#8b8f8c]">Manage inventory sales and track transactions</p>
              </div>
            </div>
          }
          icon={schema?.icon || "📦💰"}
          ...
          emptyMessage={
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-medium text-[#1f221f] mb-1">No inventory sales found</h3>
              <p className="text-sm text-[#8b8f8c]">Get started by creating your first inventory sales invoice</p>
            </div>
          }
